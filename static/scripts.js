...
async function loadRootObjects(pushHistory = true) {
    ...
    rootObjects.forEach(obj => {
        html += `<li class="clickable" onclick="loadObject(${obj.sys})">
                    ...
                    <a href="https://eth.swisscovery.slsp.ch/discovery/search?query=any,exact,(ETHUDK)${String(obj.sys).padStart(9, '0')},AND&tab=discovery_network&search_scope=DiscoveryNetwork&sortby=rank&vid=41SLSP_ETH:ETH&mfacet=institution,include,5503,1&lang=en&mode=advanced&offset=0" 
                       target="_blank" 
...
async function loadObject(sysId, pushHistory = true) {
    ...
    let html = `<h3>${data.descriptor_eng} (${data.sys}) <a href="https://eth-udk.library.ethz.ch/terms/${data.sys}/eng" target="_blank" title="Open in ETH-UDK Database" class="eth-udk-btn">ETH-UDK DB</a>&nbsp;<a href="https://eth.swisscovery.slsp.ch/discovery/search?query=any,exact,(ETHUDK)${String(data.sys).padStart(9, '0')},AND&tab=discovery_network&search_scope=DiscoveryNetwork&sortby=rank&vid=41SLSP_ETH:ETH&mfacet=institution,include,5503,1&lang=en&mode=advanced&offset=0" 
    ...
    if (data.broader_terms_resolved.length > 0) {
        ...
        data.broader_terms_resolved.forEach(bt => {
            html += `<li class="clickable" onclick="loadObject(${bt.sys})">${bt.name} (${bt.sys}) <a href="https://eth-udk.library.ethz.ch/terms/${bt.sys}/eng" target="_blank" title="Open in ETH-UDK Database" class="eth-udk-btn" onclick="event.stopPropagation();">ETH-UDK DB</a>&nbsp;<a href="https://eth.swisscovery.slsp.ch/discovery/search?query=any,exact,(ETHUDK)${String(bt.sys).padStart(9, '0')},AND&tab=discovery_network&search_scope=DiscoveryNetwork&sortby=rank&vid=41SLSP_ETH:ETH&mfacet=institution,include,5503,1&lang=en&mode=advanced&offset=0" 
    ...
    if (data.narrower_terms_resolved.length > 0) {
        ...
        data.narrower_terms_resolved.forEach(nt => {
            html += `<li class="clickable" onclick="loadObject(${nt.sys})">${nt.name} (${nt.sys}) <a href="https://eth-udk.library.ethz.ch/terms/${nt.sys}/eng" target="_blank" title="Open in ETH-UDK Database" class="eth-udk-btn" onclick="event.stopPropagation();">ETH-UDK DB</a>&nbsp;<a href="https://eth.swisscovery.slsp.ch/discovery/search?query=any,exact,(ETHUDK)${String(nt.sys).padStart(9, '0')},AND&tab=discovery_network&search_scope=DiscoveryNetwork&sortby=rank&vid=41SLSP_ETH:ETH&mfacet=institution,include,5503,1&lang=en&mode=advanced&offset=0" 
    target="_blank" title="Search in ETH Library @ swisscovery" class="swisscovery-btn" onclick="event.stopPropagation();">SWISSCOVERY</a></li>`;
        });
        html += '</ul>';
    }
    ...
    if (topPicks.length > 0) {
        ...
        topPicks.forEach(item => {
            ...
            li.innerHTML = `${item.descriptor_eng} (${item.sys}) 
                ...
                <a href="https://eth.swisscovery.slsp.ch/discovery/search?query=any,exact,(ETHUDK)${String(item.sys).padStart(9, '0')},AND&tab=discovery_network&search_scope=DiscoveryNetwork&sortby=rank&vid=41SLSP_ETH:ETH&mfacet=institution,include,5503,1&lang=en&mode=advanced&offset=0" 
